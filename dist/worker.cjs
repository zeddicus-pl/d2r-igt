var $eHIt3$util = require("util");
var $eHIt3$assert = require("assert");
var $eHIt3$tty = require("tty");
var $eHIt3$os = require("os");
var $eHIt3$path = require("path");
var $eHIt3$nodegypbuild = require("node-gyp-build");
var $eHIt3$buffer = require("buffer");
var $eHIt3$fs = require("fs");
var $eHIt3$worker_threads = require("worker_threads");
 ...
 */ function Struct() {
   ...
   */ function StructType(arg, data) {
            ...
            if (Buffer.isBuffer(arg)) {
                ...
                $eHIt3$assert(arg.length >= StructType.size, "Buffer instance must be at least " + StructType.size + " bytes to back this struct type");
                store = arg;
                arg = data;
            } else {
 ...
 */ function defineProperty(name, type) {
        ...
        $eHIt3$assert(!this._instanceCreated, 'an instance of this Struct type has already been created, cannot add new "fields" anymore');
        $eHIt3$assert.equal("string", typeof name, 'expected a "string" field name');
        $eHIt3$assert(type && /object|function/i.test(typeof type) && "size" in type && "indirection" in type, 'expected a "type" object describing the field type: "' + type + '"');
        $eHIt3$assert(type.indirection > 1 || type.size > 0, '"type" object must have a size greater than 0');
        $eHIt3$assert(!(name in this.prototype), 'the field "' + name + '" already exists in this Struct type');
        var field = {
            type: type
        };
    ...
    function recalc(struct) {
        ...
        function addType(type) {
            ...
            offset += padding;
            if (!struct.isPacked) $eHIt3$assert.equal(offset % align, 0, "offset should align");
 ...
 */ function $9945b6146eebfe67$var$useColors() {
    return "colors" in module.exports.inspectOpts ? Boolean(module.exports.inspectOpts.colors) : $eHIt3$tty.isatty(process.stderr.fd);
}
/**
 ...
 */ function $9945b6146eebfe67$var$log() {
    return process.stderr.write($eHIt3$util.format.apply($eHIt3$util, arguments) + "\n");
}
/**
 * Save `namespaces`.
 *
 ...
 */ $9945b6146eebfe67$var$formatters.o = function(v) {
    ...
    return $eHIt3$util.inspect(v, this.inspectOpts).split("\n").map(function(str) {
        return str.trim();
    }).join(" ");
};
/**
 ...
 */ $9945b6146eebfe67$var$formatters.O = function(v) {
    this.inspectOpts.colors = this.useColors;
    return $eHIt3$util.inspect(v, this.inspectOpts);
};
...
function $f340b0a3f0972096$var$supportsColor(haveStream, streamIsTTY) {
    ...
    if (process.platform === "win32") {
        ...
        const osRelease = $eHIt3$os.release().split(".");
...
module.exports = {
    ...
    stdout: $f340b0a3f0972096$var$translateLevel($f340b0a3f0972096$var$supportsColor(true, $eHIt3$tty.isatty(1))),
    stderr: $f340b0a3f0972096$var$translateLevel($f340b0a3f0972096$var$supportsColor(true, $eHIt3$tty.isatty(2)))
};
...
var __dirname1 = $eHIt3$path.resolve(__dirname, "../node_modules/ref-napi/lib");
"use strict";


var $50e14f1a427f416d$require$inspect = $eHIt3$util.inspect;

const debug = (parcelRequire("7CZVQ"))("ref");



exports = module.exports = $eHIt3$nodegypbuild($eHIt3$path.join(__dirname1, ".."));
exports.endianness = $eHIt3$os.endianness();
/**
 ...
 */ exports.get = function get(buffer, offset, type) {
    ...
    $eHIt3$assert(type.indirection > 0, `"indirection" level must be at least 1, saw ${type.indirection}`);
 ...
 */ exports.set = function set(buffer, offset, value, type) {
    ...
    $eHIt3$assert(type.indirection >= 1, '"indirection" level must be at least 1');
 ...
 */ exports.writeCString = function writeCString(buffer, offset, string, encoding) {
    $eHIt3$assert(Buffer.isBuffer(buffer), "expected a Buffer as the first argument");
    $eHIt3$assert.strictEqual("string", typeof string, 'expected a "string" as the third argument');
    if (!offset) offset = 0;
...
Object.keys(exports.alignof).forEach((name)=>{
    ...
    $eHIt3$assert(exports.types[name].alignment > 0);
});
...
if (!(exports.NULL instanceof Buffer)) {
   ...
   */ var SlowBuffer = $eHIt3$buffer.SlowBuffer;
...
module.exports.destroy = $eHIt3$util.deprecate(()=>{}, "Instance method `debug.destroy()` is deprecated and no longer does anything. It will be removed in the next major version of `debug`.");
/**
 * Colors.
 */ module.exports.colors = [
    6,
 ...
 */ function $321f4e9f5af9d6e3$var$log(...args) {
    return process.stderr.write($eHIt3$util.format(...args) + "\n");
}
/**
 * Save `namespaces`.
 *
...
function $9d39d9d74dd67909$var$CIF(rtype, types, abi) {
    ...
    $eHIt3$assert(!!rtype, 'expected a return "type" object as the first argument');
    $eHIt3$assert(Array.isArray(types), 'expected an Array of arg "type" objects as the second argument');
...
$eHIt3$assert.strictEqual($iQ7uV.FFI_TYPE_SIZE, $b4265bcf4e6acf08$var$FFI_TYPE.size);
/**
 ...
 */ function $b4265bcf4e6acf08$var$Type(type) {
    ...
    $eHIt3$assert(type.indirection >= 1, 'invalid "type" given: ' + (type.name || type));
    let ret;
    ...
    $eHIt3$assert(ret, "Could not determine the `ffi_type` instance for type: " + (type.name || type));
...
var $db72e638f525d77d$var$__dirname1 = $eHIt3$path.resolve(__dirname, "../node_modules/ffi-napi/lib");
"use strict";


var $6Wwqc = parcelRequire("6Wwqc");

$eHIt3$assert($6Wwqc.instance);

const $db72e638f525d77d$var$bindings = $eHIt3$nodegypbuild($eHIt3$path.join($db72e638f525d77d$var$__dirname1, ".."));
 ...
 */ function $13f4307a81002067$var$CIF_var(rtype, types, numFixedArgs, abi) {
    ...
    $eHIt3$assert(!!rtype, 'expected a return "type" object as the first argument');
    $eHIt3$assert(Array.isArray(types), 'expected an Array of arg "type" objects as the second argument');
    $eHIt3$assert(numFixedArgs >= 1, "expected the number of fixed arguments to be at least 1");
    // the buffer that will contain the return `ffi_cif *` instance
 ...
 */ function $4d92d74846106115$var$Function(retType, argTypes, abi) {
    ...
    // check args
    $eHIt3$assert(!!retType, 'expected a return "type" object as the first argument');
    $eHIt3$assert(Array.isArray(argTypes), 'expected Array of arg "type" objects as the second argument');
 ...
 */ function $a2dcf4b952e92dc9$var$Callback(retType, argTypes, abi, func) {
    ...
    $eHIt3$assert(!!retType, 'expected a return "type" object as the first argument');
    $eHIt3$assert(Array.isArray(argTypes), 'expected Array of arg "type" objects as the second argument');
    $eHIt3$assert.equal(typeof func, "function", "expected a function as the third argument");
    // normalize the "types" (they could be strings, so turn into real type
    // instances)
 ...
 */ function $594c1c6e6668d004$var$ForeignFunction(funcPtr, returnType, argTypes, abi) {
    ...
    // check args
    $eHIt3$assert(Buffer.isBuffer(funcPtr), "expected Buffer as first argument");
    $eHIt3$assert(!!returnType, 'expected a return "type" object as the second argument');
    $eHIt3$assert(Array.isArray(argTypes), 'expected Array of arg "type" objects as the third argument');
 ...
 */ function $cfb11f37012c71e4$var$VariadicForeignFunction(funcPtr, returnType, fixedArgTypes, abi) {
    ...
    $eHIt3$assert(Buffer.isBuffer(funcPtr), "expected Buffer as first argument");
    $eHIt3$assert(!!returnType, 'expected a return "type" object as the second argument');
    $eHIt3$assert(Array.isArray(fixedArgTypes), 'expected Array of arg "type" objects as the third argument');
...
var $88357b11b575cad0$require$read = $eHIt3$fs.readFileSync;
// typedefs
 ...
 */ function $88357b11b575cad0$var$DynamicLibrary(path, mode) {
    ...
    $eHIt3$assert(Buffer.isBuffer(this._handle), "expected a Buffer instance to be returned from `dlopen()`");
 ...
 */ $88357b11b575cad0$var$DynamicLibrary.prototype.get = function(symbol) {
    ...
    $eHIt3$assert.strictEqual("string", typeof symbol);
    const ptr = $88357b11b575cad0$var$dlsym(this._handle, symbol);
    $eHIt3$assert(Buffer.isBuffer(ptr));
...
if (0, $eHIt3$worker_threads.parentPort) {
    (0, $eHIt3$worker_threads.parentPort).once("message", (message)=>{
        if (message.type != "igt") return;
        if (message.action == "startIgt") (0, $0924226f1177aa76$export$a679630bf91eb455)();
        if (message.action == "stopIgt") (0, $0924226f1177aa76$export$cb0b9fb431e2932b)();
        if (message.action == "setTickTime") (0, $0924226f1177aa76$export$11819f9fff450d9b)(message.value);
    });
    (0, $0924226f1177aa76$export$5801d716674e6bb6)((state)=>{
        if (0, $eHIt3$worker_threads.parentPort) (0, $eHIt3$worker_threads.parentPort).postMessage({
            type: "igt",
            state: state
        });
    });
